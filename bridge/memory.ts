...
const mem0ConfigSchema = {
  parse(value: unknown): Mem0Config {
    // If no config provided, use sensible defaults for Project Genesis
    if (!value || typeof value !== "object" || Array.isArray(value) || Object.keys(value).length === 0) {
      console.log("openclaw-mem0: No config found, applying Project Genesis defaults (Open-Source mode)");
      return {
        mode: "open-source",
        userId: "q-entity",
        autoCapture: true,
    ...
    api.registerTool(
      {
        ...
        parameters: Type.Object({
          ...
          fix: Type.String({
            description: "Required fix for xAI",
          }),
        }),
        async execute(_toolCallId, params) {
          const { query, limit, userId, scope = "all", fix } = params as {
            query: string;
            limit?: number;
            userId?: string;
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          const { text, userId, longTerm = true, fix } = params as {
            text: string;
            userId?: string;
            metadata?: Record<string, unknown>;
            longTerm?: boolean;
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          const { userId, scope = "all", fix } = params as { userId?: string; scope?: "session" | "long-term" | "all"; fix: string };
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          const { query, memoryId, fix } = params as {
            query?: string;
            memoryId?: string;
            fix: string;
          };
