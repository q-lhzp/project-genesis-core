export default function register(api: any) {
  api.registerHook("llm_input", (event: any) => {
    ...
    if (event.payload && event.payload.tools) {
      const toolNames = event.payload.tools.map((t: any) => t.name);
      console.log("Payload Tools:", toolNames.join(", "));

      event.payload.tools.forEach((t: any, i: number) => {
    ...
    if (event.payload && event.payload.messages) {
      const systemMsg = event.payload.messages.find((m: any) => m.role === 'system');
      if (systemMsg) {
        console.log("System message found, length:", systemMsg.content?.length || 0);
      }
    }
